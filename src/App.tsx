...
const { HEIGHT, WIDTH, WAVE_BUFFER_SIZE, TIME_BUFFER_LENGTH, FREQ_BUFFER_LENGTH } = CANVAS;
...
function App() {
  ...
  const waveBufferRef = useRef<number[]>(new Array(WAVE_BUFFER_SIZE * TIME_BUFFER_LENGTH).fill(128));
  ...
  const draw = useCallback(() => {
    ...
    waveBufferRef.current = [...waveBufferRef.current.slice(TIME_BUFFER_LENGTH), ...Array.from(timeDataArray)];
    ctx.lineWidth = 2;
